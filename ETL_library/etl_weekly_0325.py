        ...
        with open(f"{log_path}{item}.log", "w") as etl_log, contextlib.redirect_stdout(etl_log), contextlib.redirect_stderr(etl_log):
            try:
                ...
                pog_df = gen_pog_and_date(item=item,
                                        ...
                                        pog_path='scai.pog_0322',
                                        test_path='/app/python-scripts/kevin_workspace/assist_data/test_sample0325.csv.gz')

                print(f"Loading sales data...")
                sales_df = getsaletable(item=item, path='scai.item_sales_0322')
                ...
                res_df = get_prom(item=item, data=df, prom_info=prom_info, date_range=('2019-01-01', '2021-04-28'))
                ...
                res_df['store_id'], res_df['item_code'] = res_df['store_id'].astype(str), res_df['item_code'].astype(str)
                common_feat = list(set(res_df.columns) & set(features))
                print("ENCRYPT FEATURES: " + str(common_feat))

                for feat in common_feat:
                    print('ENCRYPTING ' + feat)
                    feat_mapper = get_mapper(feat)
                    res_df = res_df.merge(feat_mapper, how='left', on=feat)

                # DROP COLUMNS
                res_df.drop(common_feat, axis=1, inplace=True)
                print("FEATURES: " + str(list(res_df.columns)))

                # Export data
                print(f"THE RESULT DATA HAS {len(res_df)} ROWS")
                print(f"Exporting {item} at {now()}...")
                res_df.to_csv(f"{export_path}{item}.csv.gz",compression='gzip',index=False,header=False)
                print(f"exported to {export_path} at {now()}")

            except:
                error = traceback.format_exc()
                print(error)
                if 'impala' in error or 'MemoryError' in error:
                    time.sleep(10)
                    print("RECONNECT TO IMPALA IN 10 SECONDS...")
                else:
                    try_again = False
...
while retry:
    try:
        #* GET ITEM LIST
        data = impala_query(f"select distinct item_code from scai.item_sales_0322")
        item_list = list(data.item_code)
        # item_list = ['100006252']
        exported = [i[:i.index('.csv.gz')] for i in os.listdir('/app2/kevin_workspace/data0325_encrypt/')]
        item_list = [i for i in item_list if i not in exported]
        # print(item_list)

        #* DEFINE EXPORT PATH
        export_path = '/app2/kevin_workspace/data0325_encrypt/'
        log_path = '/app/python-scripts/kevin_workspace/etl_library/log0325_weekly/'
        ...
        error = traceback.format_exc()
        if 'impala' in error or 'Memory' in error:
            time.sleep(10)
