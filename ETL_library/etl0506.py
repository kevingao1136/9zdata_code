        ...
        with open(f"{log_path}{item}.log", "w") as etl_log, contextlib.redirect_stdout(etl_log), contextlib.redirect_stderr(etl_log):
            try:
                ...
                pog_df = gen_pog_and_date(item=item,
                                        ...
                                        pog_path='scai.0513_0602_pog',
                                        test_path='/app/python-scripts/kevin_workspace/assist_data/test_sample0506.csv.gz')

                print(f"Loading sales data...")
                sales_df = getsaletable(item=item, path='scai.0513_0602_item_sales')
                ...
                res_df = get_prom(item=item, data=df, prom_info=prom_info, date_range=('2019-01-01', '2021-08-25'))
...
#* GET ITEM LIST
data = impala_query(f"select distinct item_code from scai.0513_0602_item_sales")
item_list = list(data.item_code)
# item_list = ['100006252']
exported = [i[:i.index('.csv.gz')] for i in os.listdir('/app2/kevin_workspace/data0506_encrypt/')]
item_list = [i for i in item_list if i not in exported]

#* DEFINE EXPORT PATH
export_path = '/app2/kevin_workspace/data0506_encrypt/'
log_path = '/app/python-scripts/kevin_workspace/etl_library/log0506/'
