...
def reduce_mem_usage(props):
    ...
    # log("Memory usage of properties dataframe is :",start_mem_usg," MB")
    NAlist = [] # Keeps track of columns that have missing values filled in. 
    for col in props.columns:
        if props[col].dtype != object and props[col].dtype != 'datetime64[ns]':  # Exclude strings
            
            # log current column type
            # log("******************************")
            # log("Column: ",col)
            # log("dtype before: ",props[col].dtype)
            ...
            # log new column type
            # log("dtype after: ",props[col].dtype)
            # log("******************************")
    
    # log final result
    # log("___MEMORY USAGE AFTER COMPLETION:___")
    mem_usg = props.memory_usage().sum() / 1024**2 
    # log("Memory usage is: ",mem_usg," MB")
    # log("This is ",100*mem_usg/start_mem_usg,"% of the initial size")
    return props, NAlist
...
def get_prom(item, data, prom_info, date_range):
    ...
    """
    # GET PROM INFO
    assert item in prom_info.item, "ITEM NOT IN PROM_INFO DATA"
    ...
    # log(data[['day_dt','item_code']].dtypes)
    # log(cross_join_prom[['day_dt','item']].dtypes)
...
if __name__ == '__main__':
    ...
    #* DEFINE EXPORT PATH
    export_path = '/app/python-scripts/kevin_workspace/regular_shampoo_items/'
    log_path = '/app/python-scripts/kevin_workspace/regular_shampoo_items/log/'
    # export_path = './'

    #* GET ITEM LIST
    dim_product = pd.read_csv("/app/python-scripts/kevin_workspace/assist_data/dim_product.csv")
    item_list = list(dim_product.query("class_desc == 'Shampoo' and sbclass_desc == 'Regular'").item_idnt)[:100]
