...
def load_data(item_path, train_start, val_end, test_start, test_end):
    ...
    Returns:
        ...
        test_df: test data
    """
    df = pd.read_csv(item_path, compression='gzip', parse_dates=['day_dt'], usecols=[
        'day_dt', 'day_type', 'mday', 'yday', 'year', 'month', 'weekday', 'is_5th', 
    'last_day_type', 'area_idnt', 'loc_fmt_cde', 'loc_selling_area', 'loc_tot_area', 
    'pds_location_type_en', 'pds_mtg_type', 'pds_store_segmentation', 'pds_grace', 
    'pds_floor_type', 'city_tier', 'zone_id', 'is_intracity_dlvr_store', 'loc_wh', 
    'is_central_store', 'ttl_quantity', 'avg_amountofsales', 'ttl_amt_payment', 
    'item_brand', 'offer_code', 'osd_type', 'prom0', 'prom1','prom2','prom3','prom4',
    'prom5','prom6','prom7','is_vip', 
    'free_gift', 'unit_price', 'required_num', 'amount', 'prom_price', 'retail_price', 
    'p_rate', 'regn_idnt', 'activity_desc', 'item_code', 'distt_idnt',
    'store_type', 'mkt_idnt', 'store_id'
    ])

    # print(df.head())
    # print(df.columns)
    # df.columns = ['day_dt', 'day_type', 'mday', 'yday', 'year', 'month', 'weekday', 'is_5th', 
    # 'last_day_type', 'area_idnt', 'loc_fmt_cde', 'loc_selling_area', 'loc_tot_area', 
    # 'pds_location_type_en', 'pds_mtg_type', 'pds_store_segmentation', 'pds_grace', 
    # 'pds_floor_type', 'city_tier', 'zone_id', 'is_intracity_dlvr_store', 'loc_wh', 
    # 'is_central_store', 'ttl_quantity', 'avg_amountofsales', 'ttl_amt_payment', 
    # 'item_desc', 'sbclass_desc', 'item_brand', 'offer_code', 'osd_type', 'x件y折', 
    # 'x元y件', '加x元多y件', '买x送y', '满x减y', 'x件减y', '第x件y折', '换购', 'is_vip', 
    # 'free_gift', 'unit_price', 'required_num', 'amount', 'prom_price', 'retail_price', 
    # 'flag', 'p_rate', 'regn_idnt_new', 'activity_desc_new', 'item_code_new', 'distt_idnt_new', 
    # 'store_type_new', 'mkt_idnt_new', 'store_id_new']
    ...
    train_val_df.area_idnt, unit_price.area_idnt = train_val_df.area_idnt.astype(int), unit_price.area_idnt.astype(int)
    train_val_df.item_code, unit_price.item_code = train_val_df.item_code.astype(str), unit_price.item_code.astype(str)
    train_val_df = train_val_df.merge(unit_price, on=['item_code','area_idnt','day_dt'], how='left', indicator=True)
    if 'both' not in train_val_df._merge.astype(str).unique(): print('NO UNIT PRICE DATA WAS MERGED >>>>>>>>>>>>>>>>>>>>>>')
    train_val_df.drop(['_merge'],axis=1,inplace=True)
...
def export_results(item_code, train, val1, val2, test, export_result_meta_data, export_img):
    '''
    Parameters:
        item_code - item code
        train - train dataset
    ...
    if export_result_meta_data:
        ...
        meta_data['test_start_time'] = ['2021-03-25']
        meta_data['test_end_time'] = ['2021-04-28']
...
data_path = f"/app2/kevin_workspace/data0303/"
output_path = f"/app/python-scripts/kevin_workspace/outputs/output0325_weekly/"
log_path, result_data_path, meta_data_path, img_path = output_path+'log/', output_path+'result_data/', output_path+'meta_data/', output_path+'train_img/'

#* TEST PATH
# img_path = log_path = result_data_path = meta_data_path = '/app/python-scripts/kevin_workspace/test/'
...
test_start, test_end = '2021-03-25', '2021-04-28'
...
#* DEFINE ITEM LIST
item_list = [i[:i.index(".csv.gz")] for i in os.listdir(data_path)]
...
if __name__ == '__main__':
    ...
    unit_price = pd.read_csv('/app/python-scripts/kevin_workspace/assist_data/unit_price_weekly_0325.csv.gz',parse_dates=['day_dt'],compression='gzip')
